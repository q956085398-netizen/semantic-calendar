...
- 窗口尺寸：最小 820×560，小于左右栏的自动折叠阈值（1080 / 880），因此缩到最小尺寸时两侧面板都已收起、月历主体独占整宽，折叠栏以悬浮按钮提供临时展开（ui-design §23）。这条跨文件的尺寸契约由 `src/layout/window-size-contract.test.ts` 守住，同一份文件的两端都锁着——最小尺寸（SC-002）与默认尺寸（SC-021 / ui-design §26 第 4 项：1180×760 下三栏都展开，月历区仍是最大的一栏）；
...
- 数据校验（开发期一次性，脚本未入库）：推导结果对全部 73,049 天与官方年表逐日回环一致；另与表驱动实现 `lunardate` 和天文计算实现 `lunar-javascript` 交叉比对，分歧只有四处，都以官方数据为准——1933 / 1954 / 1978 年（旧版农历表在这三年偏差一天）与 2057 年九月月首（天文实现偏差一天）。这四处连同春节、闰月月首、范围两端共 29 个日期固定为测试样例（`providers/china/lunar.test.ts`），官方数据一旦被改动测试会立刻报错；另有 14 条「公众日历样例」（春节 / 元宵 / 端午 / 中秋与闰月月首）单独登记，期望值是民用日历上的常识日期，不依赖推导工具与年表文本，用来抵消「表与实现同源」的盲区；
...
- UI：月格在日期数字下加一行农历（ui-design §5.2），用 `--text-secondary` 而不是更淡的 `--text-muted`（§24 对比度，层级靠 15px/600 的数字与 11px 的农历表达），跨月格同样显示、随跨月样式弱化；详情栏按 §12 的次序排在公历日期与年份之后（「9月24日 / 2026 / 农历八月十四」）。两处都只渲染注入的文本，组件里没有任何农历判断。§5.2 的「字号与字重明显低于公历日数字，长月名不得挤压或推移日期数字」由 `calendar/lunar-cell-fit-contract.test.ts` 守住（jsdom 不做布局，SC-021 的验收文档 §5.1 记的就是这类漏检）：日期数字与农历是列里的两行、各自成盒，农历行留在流式排版里且不换行（溢出走省略号），字重链上没有意外加粗，字体栈里留着中日韩字族（全角系数 1 字 = 1em 的依据）；算术上，最长的农历文本「闰十一月」在桌面窗口能产生的最窄格子（双栏展开的下限 1081px 窗口、67.6px 格子；再窄一像素右栏就收成 rail、格子反而变宽）里需要 44px、可用 47.6px——字号 / 字重 / 内边距 / 换行规则改坏时它会红。同一组数字另经一次真实渲染对照（临时装置未入库，与 §5.1 的复检同一办法）：最窄格子里农历行没有触发省略号，把「初九」换成「闰十一月」时日期数字的盒子逐像素不变（47.56 × 18px），字重 600 对 400。
...
- 数据核对（开发期一次性）：三条安排逐条对回通知原文（2026-09-25 重读三个 `sourceUrl` 页面）——放假区间、补班日、条目名、文号与发布日期全部一致；另有一张逐条读自通知原文的样例表固定为测试，`providers/china/holidays.test.ts` 的「通知口径样例」一组按天检查区间内是放假日、补班日是补班日、区间首尾之外不是放假日，并检查样例覆盖到每一年每一条通知条目（只加数据不加样例会红）。样例表不读数据文件，因此两处必须各改一次——实测：把 2025 劳动节补班改成 4 月 26 日、把 2025 国庆中秋区间整体挪一天（天数不变），都只有这一组会红；改 2025 年的文号只有版本清单那一条会红（`providers/china/holidays.test.ts` 的「版本清单逐条登记文号、发布日期、来源地址与修订号」）；改动放假日总数（例如区间尾多写一天）则会连同既有的「放假日数」与装配期校验一起红。放假天数与补班日数与通知口径一致（2024：32 天 / 8 天，2025：28 / 5，2026：33 / 6，含通知写「与周末连休」的周末）；
...
SC-011 的边界：月格的休假视觉（连续背景、被格子裁切的「休」「补」大字、多语义冲突规则）由 SC-013 实现，本 Ticket 只交付数据、连休分组、格子挂钩与文字层；数据覆盖 2024–2026，2027 年安排（通常于 2026 年 11 月发布）尚未发文，因此未登记，未登记年份安静地不显示休 / 补，不推算（P-03）；2024 年除夕当年只是「鼓励」休假、不是法定假日，因此 2024-02-09 不在数据里（2025 年起除夕为法定假日）；`revision` 用于标记数据修正（只改数据不改年份时递增），已登记的通知文号 / 发布日期 / 来源地址由 `versions` 暴露，供追溯（CN-007）——这份清单本身逐条锁在 `providers/china/holidays.test.ts` 的「版本清单逐条登记文号、发布日期、来源地址与修订号」里，改文号、改发布日期或推进修订号都必须显式改测试。

传统节日与二十四节气（SC-012）与农历、法定节假日同目录，位于 `apps/desktop/src/providers/china/`，为月格与详情栏提供日级语义（CN-005–006）：

- 传统节日 `providers/china/festivals.ts`：`chinaFestivals.festivalsOfKey(dateKey)` 给出当日的节日（普通日期是空数组）。日期不抄表，判定规则只有三种历法事实——农历固定月日（春节、元宵、龙抬头、端午、七夕、中元、中秋、重阳、腊八）、农历年的最后一天（除夕：腊月 29 还是 30 天由年数据决定，不写死）、节气日（清明节＝清明，与节气共用同一张表）；闰月不算节日（闰五月初五不是端午）。装配期校验重名、重规则、越界日期与不存在的节气 id，空表直接抛错；
- 二十四节气 `providers/china/solar-terms.ts` + `solar-terms-data.ts`：`solarTermOfKey(dateKey)` 回答「这一天是哪个节气」，只做查表不做天文计算（节气日期没有可用的简单公式，只有数据）。表由 `tools/derive-solar-terms.mjs` 从香港天文台「公曆與農曆日期對照表」年表的「節氣」列推导，与农历表取自同一批年表（繁体名只用于解析，展示口径是简体）；覆盖公历 1901–2100，每个公历年 24 个日期、月份由序号决定，因此只存日不存月；表外年份返回 undefined，界面安静地不显示，不推算（P-03）；数据表一行一年、已列入 `.prettierignore`；
- 数据生成与核对：2026-09-25 用 1901–2100 全部 200 份官方年表重跑推导工具，输出与已提交的 `solar-terms-data.ts` **逐字节一致**；`providers/china/solar-terms-derive.test.ts` 把「生成」这一步固定成可执行的：输入是手写的年表片段（UTF-8 BOM、CRLF、`公曆日期 農曆日期 星期 節氣` 四列），断言输出的年份范围、24 个日期、节气清单与数据来源口径，再用 `solarTermOfKey` 把生成的日期按序号读回来（工具与 Provider 的清单顺序不能各说各话），并覆盖五条失败路径——缺项、月份错位、日期越界、同年重复、年份不连续都必须以非零退出码失败且不写出输出文件。「已提交的 2026 行就是这份输入生成的结果」一条把工具与已提交的数据焊在一起：改工具让 2026 行变了就会红；「已提交的 200 行整体锁定」再锁一层——全表摘要与年份范围（1901–2100）写死在测试里，手改任何一年都得重新生成并显式更新摘要（摘要是变更探测器，正确性靠重新生成时核对年表，年表文本不随仓库分发）。这一轮同时修掉工具的一个真问题：一年都没通过校验时它会读空的 `entries[0]` 崩掉，把「2026 年只有 23 个节气」这类诊断顶替成一段栈，现在先报问题再按退出码决定成败；
- 文本 `providers/china/festival-labels.ts` / `solar-term-labels.ts`：节日给名称、英文名与一句释义，节气再多一个序号（「第 19 个节气」）；背景是逻辑引用（`bg.festival.mid-autumn-festival` / `bg.solar-term.cold-dew`），由资源包解析——仓库不分发图片二进制，取不到就是不画背景，日期与语义文字照常显示；
- 接线与 UI：`semantic/app-china-festivals.ts` 绑成「日期键 → entries」交给月视图与详情栏，entries 的顺序即优先级（节日在前、节气在后，§16.1；清明节当天两条都成立，月格只画一个主背景）；月格里节日名占农历那一行、节气名做右上角小标签，详情栏给出名称、英文名、序号与一句释义（§9.2）；背景引用经 `semantic/day-backdrop.ts` 解析后由 SC-013 的 `calendar/cell-backdrop.ts` 裁决要不要画。与农历、法定节假日同一条边界：日级语义不进事件 Matcher Engine（输入是事件，而「这一天是中秋」与有没有事件无关），UI 里不出现任何节日 / 节气名称与 id（`ui-boundary.test.ts`）；
- 测试：日期准确性在 `providers/china/festivals.test.ts`（官方年表样例、除夕恒为春节前一天且落在腊月最后一天、闰月不算节日、范围外安静降级、装配校验）与 `providers/china/solar-terms.test.ts`（官方年表样例、每个公历年恰好 24 个节气且按序号递增、每个节气落在自己的公历月与常见窗口内、范围外安静降级、表结构校验），月格与详情栏的呈现分别在 `calendar/MonthView.test.tsx` 的「月格节日 / 节气」一组与 `layout/InspectorPanel.test.tsx` 的「详情栏节日 / 节气块」一组，端到端（内置数据 → 月格 → 详情栏）在 `App.test.tsx` 的「节日与节气在月格与详情栏里显示」一条。

SC-012 的边界：只登记日期无争议的节日——小年（北方廿三 / 南方廿四）与寒食（清明前一二日）随地区与流派变化，不登记、也不猜一个（P-03）；没有照片资源，专属背景是能力而非默认画面（SC-022 定下 v0.1 口径，见 [third-party-assets.md](docs/third-party-assets.md) §1）；节气日期表只由推导工具生成，范围扩展或数据修正时重跑工具并补官方样例，不手改数据文件。
...
测试与人工验收（SC-021）把各单的测试对齐成一处可检查的清单，并补上四处缺口。验收清单与记录在 [docs/ui-acceptance.md](docs/ui-acceptance.md)：§26 的 18 条逐项给出判据、自动证据（测试文件与用例名）与人工记录，附 4 张整屏截图（浅色 / 深色 / 国庆连休 / 最小窗口），SC-021 的六条验收项也在同一份文档里对齐：
...
- 默认窗口布局从「只有人工记录」变成可执行断言：§26 第 4 项（主月历在默认窗口尺寸下占据主要空间）此前在验收表里的自动一栏是「—（布局事实，jsdom 无布局）」，但它其实与 SC-023 那条一样可以做算术——`layout/window-size-contract.test.ts` 从 `tauri.conf.json` 取默认窗口宽度、从 `index.css` 取栏宽与间距 token（都不另抄一份），断言 1180×760 下三栏都展开、月历区比两侧栏之和（520px）还宽、且占窗口宽度的一半以上，并断言骨架满高、月历区自己不留内边距与边框（§2「优先保证中央月历空间」；两侧栏有内边距 / 边框作为对照）。把侧栏调宽到月历不再压过两侧之和、默认窗口调窄到触发折叠、或把折叠阈值抬到默认宽度以上，它都会失败。口径写在测试文件与 `docs/ui-acceptance.md` §4 第 4 项：§2 的三栏比例（中央 62%–66%）是建议值，当前固定 token 在默认窗口下是 52.5%，锁的是 §26 第 4 项的验收措辞而不是建议比例；
