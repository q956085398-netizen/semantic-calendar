...
> 当前实现（SC-018）：第 5 项是一个「设置」入口按钮，点击后在月历主区域打开
> 设置页（外观 / 区域 / 数据源 / 关注球队 / 通知 / 关闭窗口时 / 关于），顶部有「返回月视图」。
> 因此侧栏不再重复这些设置控件：数据源列表保留「显示 / 隐藏」开关与刷新，
> 需要确认或较少改动的管理动作（删除来源、WebCal 刷新间隔）与主题、通知、
> 关闭行为一起集中在设置页——同一处状态只有一个控件，两处界面都不会各写一份。
...
> 导入的 ICS 文件与 WebCal 订阅排在内置四行之前，各自的 checkbox 是该来源的启用状态；
> 行内状态文字仍写清“已停用 / 刷新失败：…”等事实（SRC-003），不靠勾选框单独传达。
