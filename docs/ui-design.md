...
关闭窗口行为（SC-002）用两项分段控件表达「隐藏到托盘 / 退出应用」，
并在控件下方写明该行为意味着“应用是否仍在运行”——状态不靠颜色单独传达。
文案用“隐藏”而不是“最小化”：最小化按钮仍是系统默认行为（最小化到任务栏），
只有关闭窗口才会走到托盘。

> 当前实现（2026-09-26 修订）：侧栏保留品牌、小月历、四个内置显示开关、导入与订阅日历的名称、关注球队与设置入口。
> 侧栏只提供显示开关；订阅地址、导入、新增、重命名、刷新、删除及错误详情集中到设置页。
> 新增订阅可填写自定义名称，已有来源也可重命名；旧版以地址作名称的订阅在侧栏显示为“订阅日历”，不展示长地址。
> 设置中的地址默认折叠并省略账号和查询凭据；内置显示选项的完整说明也默认折叠，日常切换在侧栏完成。
> 单击“设置”打开，再次单击返回月历；顶部返回按钮也可使用。设置页隐藏日期详情，
> 在最大 1160px 的内容宽度内分成“偏好设置”和“日历与提醒”两列独立分组，窄窗口改为单列。
> 侧栏图标使用“历”字。普通月历没有多余滚动条；内容确实超过窗口时使用细、低对比滚动条。
...
> 当前实现（SC-018）：四行就是内置来源的显示开关（checkbox 的可访问名称即来源名），
> 关闭只影响展示、不删数据——关掉「中国节假日」不会清掉假期数据，重新勾选立即恢复。
> 导入的 ICS 文件与 WebCal 订阅在四行之后单独分组，各自的 checkbox 是该来源的启用状态；
> 名称过长时省略，悬停显示完整名称。来源状态文字在设置中写清“已停用 / 刷新失败：…”等事实（SRC-003）。
> 小窗口或日历较多时，只滚动日历控制列表；设置入口和收起按钮固定留在底部，不能重叠。
...
> 当前实现（SC-020）：这一行还可以带一条**整理事件**的状态行（`正在整理事件（N 条）…`），
> 只在大数据量下分片读取未完成时出现，与导航同一行、同样式族的次要文字，因此不新增表头
> 高度（§26 第 5 项的最小窗口验收不受影响）。文案由 App 给出，月视图不判断何时该显示
> （见 `calendar/month-occurrences.ts` 的 `monthOccurrencePendingText`）。
...
> 2026-09-26 降级修正：节日、节气名称本身不抢占背景。只有图片实际可用时才优先使用专属视觉；资源缺失或加载失败时，保留休假/补班底色及大字。
