...
| 10 | 中秋、寒露、霜降等可显示专属背景 | `display/DayBackdrop.test.tsx`（图片 + 遮罩、缺图与加载失败降级、资源包抛错不外泄）；`calendar/MonthView.test.tsx`「节日专属背景：图片被格子裁切，假期视觉整体让位」 | 仓库不分发图片资源，**本次看到的是降级形态**：语义文字与节气标签（10-08「寒露」、10-23「霜降」、09-25「中秋节」），照片背景未渲染 | 部分通过（能力由测试覆盖；SC-022 已定口径：v0.1 不接入照片资源，见 §5.2） |
