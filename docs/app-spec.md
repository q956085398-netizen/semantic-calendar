...
> 当前实现（SC-014）：球队字典 `apps/desktop/src/providers/football/teams.ts` 保存 20 支球队的稳定 ID、中英文名、常见别名、3 字母代码、近似球队色与 `crestRef` 逻辑引用；联赛元数据与赛季参赛名单在 `competitions.ts`（名单是独立数据条目，新赛季追加即可，Matcher / Resolver / UI 不改）；装配与查询在 `football-catalog.ts`，装配期校验别名唯一、已规范化且跨队不冲突，坏数据直接抛错。
>
